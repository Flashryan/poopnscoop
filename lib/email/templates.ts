...
export function businessNotificationEmail(enquiry: Enquiry) {
  ...
  const html = `<p>New enquiry received.</p><ul><li>Name: ${enquiry.full_name}</li><li>Postcode: ${enquiry.postcode}</li><li>Plan: ${enquiry.plan_type}</li><li>Extras: ${enquiry.extra_visits}</li><li>Payment status: ${enquiry.payment_status}</li></ul><p><strong>Reference:</strong> ${enquiry.id}</p>`;
  return { subject, text, html };
}
